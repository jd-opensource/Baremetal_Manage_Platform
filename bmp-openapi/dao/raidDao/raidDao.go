...
// Id doesn't exist
func GetRaidById(logger *log.Logger, raidId string) (v *Raid, err error) {
	v = &Raid{}
	err = dao.Where(logger, dao.IronicRdb, "raid_id = ? and is_del = 0", raidId).Take(v).Error
	if err != nil {
		return nil, err
	}
	return v, nil
...
func UpdateRaidById(logger *log.Logger, m *Raid) (err error) {
	...
	//m.UpdateTime = time.Now()
	return dao.Model(logger, dao.IronicWdb, Raid{}).Where("raid_id = ?", m.RaidID).Updates(m).Error
}
...
type DRaid struct {
	...
	SystemPartitionCount int    `json:"system_partition_count"`
	AvailableValue       int    `json:"available_value"`
	DiskType             string `json:"disk_type"`
}

func GetAllRraidDevice(logger *log.Logger, raid_id, device_type, volume_type string) (ml []*DRaid, err error) {

	sql := `select t1.uuid as raid_id, t1.name_en, t1.name_zh, t1.description_zh, t1.description_en, t2.device_type, t2.volume_type, t2.volume_detail, t2.system_partition_count,t2.available_value,t2.disk_type from raid t1 left join r_device_type_raid t2 on t1.uuid = t2.raid_id where t1.is_del = 0 and t2.is_del = 0`
	if raid_id != "" {
...
func GetAllRaidByVolumeId(logger *log.Logger, volumeId string) (ml []*Raid, err error) {
	sql := `select a.* from raid a inner join r_volume_raid b on a.raid_id = b.raid_id where 1=1`

	sql = sql + fmt.Sprintf(" and b.volume_id = '%s'", volumeId)
