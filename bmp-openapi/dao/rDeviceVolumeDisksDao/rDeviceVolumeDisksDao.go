...
type RDeviceVolumeDisks struct {
	...
	VolumeType  string `gorm:"column:volume_type;type:varchar(255);not null" json:"volume_type"`               // data|system
...
// 	err = tx.Begin().Error
// 	if err != nil {
// 		logger.Warn("AddMultiRDeviceVolumeDisk Begin error")
// 		return
// 	}
// 	for _, r := range m {
...
// 			logger.Warnf("AddMultiRDeviceVolumeDisk Create error:%s", err.Error())
// 		}
// 	}
// 	if err := tx.Commit().Error; err != nil {
// 		logger.Warnf("AddMultiRDeviceVolumeDisk Commit error:%s", err.Error())
// 		goto ROLLBACK
// 	}
...
// ROLLBACK:
// 	tx.Rollback()
// 	logger.Warnf("AddMultiRDeviceVolumeDisk rollback")
// 	if len(m) > 0 {
// 		logger.Warnf("AddMultiRDeviceVolumeDisk rollback, device_id:%s", m[0].DeviceID)
// 	}
// 	return 0, errors.New("AddMultiRDeviceVolumeDisk Commit error")
...
func AddMultiRDeviceVolumeDisk(logger *log.Logger, m []*RDeviceVolumeDisks) (id int64, err error) {
	...
	for _, v := range m {
		_, err := dao.CreateAndGetId(logger, dao.IronicWdb, v)
		if err != nil {
			logger.Warn("AddMultiRDeviceVolumeDisk.CreateAndGetId ", v.DeviceID, v.DiskID, v.VolumeID)
		}
	}
	return
