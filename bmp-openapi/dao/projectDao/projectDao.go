...
// GetProjectByUuid retrieves Project by Uuid. Returns error if
// Id doesn't exist
func GetProjectCountGroupByUser(logger *log.Logger) ([]*UserProjectCount, error) {
	ml := []*UserProjectCount{}
	sql := `select user_id, count(1) as count from project group by user_id, is_del having is_del = 0`
...
func GetAllProjectIncludeShare(logger *log.Logger, user_id string, owned int) (ml []*AllProject, err error) {
	...
	leftsql := fmt.Sprintf(`select a.is_default, a.project_id, a.project_name, a.user_id, a.created_time, 1 as owned from project a where a.is_del = 0 and a.user_id = '%s'`, user_id)
	rightsql := fmt.Sprintf(`select 0 as is_default, b.project_id, b.project_name, b.shared_user_id as user_id, b.created_time, 2 as owned from sharing_project b where b.is_del = 0 and b.shared_user_id = '%s'`, user_id)
	var sql string
	if owned == 1 { //只查有拥有权的项目
...
func UpdateProjects(logger *log.Logger, query map[string]interface{}, updates map[string]interface{}) (err error) {
