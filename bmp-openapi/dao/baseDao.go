...
func (g *gLogger) Print(values ...interface{}) {
	...
	logger := clog.New(logPath + "/bmp-openapi-sql.log")
	defer logger.Flush()
...
func WhereBuild(g *gorm.DB, query map[string]interface{}) (*gorm.DB, error) {
	logger := log.New("./log/ironic-api.log")
	query["is_del"] = baseLogic.IS_NOT_DEL
	for k, v := range query {
		...
		if len(ks) > 2 {
			...
			logger.Warn(fmt.Errorf("Error in query condition: %s. ", k))
