...
type UserInstanceCount struct {
	UserID string `json:"user_id"`
	Count  int64  `json:"count"`
}

func GetInstanceCountGroupByUser(logger *log.Logger) ([]*UserInstanceCount, error) {
	ml := []*UserInstanceCount{}
	sql := `select user_id, count(1) as count from instance group by user_id, is_del having is_del = 0`
