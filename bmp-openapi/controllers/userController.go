...
func (c *UserController) CreateUser() {
	...
	req := &requestTypes.CreateUserRequest{}
	...
	uuid, err := userLogic.CreateUser(c.logPoints, req)
	if err != nil {
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
	c.Res.Result = response.UserId{
		UserId: uuid,
	}
}
...
// DeleteUser ...
// swagger:route DELETE /users/{user_id} user deleteUser
// DeleteUser 删除用户
//     Responses:
//       200: deleteUser
//       default: ErrorResponse
...
func (c *UserController) DeleteUser() {
	defer c.CatchException()
	userId := c.Ctx.Input.Param(":user_id")
	if err := userLogic.DeleteUser(c.logPoints, userId); err != nil {
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
	c.Res.Result = response.CommonResponse{
