...
func (c *InstanceController) StopInstances() {
	c.logPoints.Warn("InstanceController.StopInstances comming", c.Ctx.Input.Param(":param"), string(c.Ctx.Input.RequestBody))
	defer c.CatchException()
	instancesAndOperation := c.Ctx.Input.Param(":param")
...
func (c *InstanceController) QueryInstanceBySn() {
	defer c.CatchException()
	sn := c.GetString("sn")
	res, err := instanceLogic.GetInstanceBySn(c.logPoints, sn)
	if err != nil {
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
