...
func CreateRole(logger *log.Logger, param *requestTypes.CreateRoleRequest) (string, error) {
	list, _ := roleDao.GetAllRole(logger, map[string]interface{}{
		"role_name_cn": param.RoleNameCn,
	})
	if len(list) != 0 {
		logger.Warn("roleNameCn exist:", param.RoleNameCn)
		panic(constant.BuildInvalidArgumentWithArgs(fmt.Sprintf("角色中文名称 %s 已存在", param.RoleNameCn), fmt.Sprintf("roleNameCn %s exist", param.RoleNameCn)))
	}
	list, _ = roleDao.GetAllRole(logger, map[string]interface{}{
		"role_name_en": param.RoleNameCn,
	})
	if len(list) != 0 {
		logger.Warn("roleNameEn exist:", param.RoleNameEn)
		panic(constant.BuildInvalidArgumentWithArgs(fmt.Sprintf("角色英文名称 %s 已存在", param.RoleNameEn), fmt.Sprintf("roleNameEn %s exist", param.RoleNameEn)))
	}
	roleEntity := &roleDao.Role{
		RoleNameEn:    param.RoleNameEn,
		RoleNameCn:    param.RoleNameCn,
		DescriptionEn: param.DescriptionEn,
		DescriptionCn: param.DescriptionCn,
	...
	roleEntity.RoleID = commonUtil.GetRandString("role-", namespacePrefix.INSTANCE_ID_LENGTH, false, true, true)
	if _, err := roleDao.AddRole(logger, roleEntity); err != nil {
		logger.Warnf("CreateRole AddRole sql error, entity:%s, error:%s", util.ObjToJson(roleEntity), err.Error())
		return "", err
	}
	return roleEntity.RoleID, nil
}

func ModifyRole(logger *log.Logger, param *requestTypes.ModifyRoleRequest, roleId string) error {
	_, err := roleDao.GetRoleById(logger, roleId)
	if err != nil {
		logger.Warn("GetRoleByUuid sql error:", roleId, err.Error())
		panic(constant.BuildInvalidArgumentWithArgs(constant.NOT_FOUND.Messagech, constant.NOT_FOUND.MessageEn))
	}
	...
	if err := roleDao.UpdateRoleById(logger, entity); err != nil {
		logger.Warn("ModifyRole UpdateRoleById sql error:", roleId, err.Error())
		return err
	}
	return nil
}

func DeleteRole(logger *log.Logger, roleId string) error {
	entity, err := roleDao.GetRoleById(logger, roleId)
	if err != nil {
		logger.Warn("GetRoleByUuid sql error:", roleId, err.Error())
		panic(constant.BuildInvalidArgumentWithArgs(constant.NOT_FOUND.Messagech, constant.NOT_FOUND.MessageEn))
	}
	userCount, err := userDao.GetUserCount(logger, map[string]interface{}{
		"role_id": roleId,
		"is_del":  baseLogic.IS_NOT_DEL,
	})
	if userCount != 0 {
		panic(constant.BuildInvalidArgumentWithArgs("角色下面含有用户，不能删除", "this role contains users,cannot delete"))
	}
	entity.IsDel = 1
	...
	if err := roleDao.UpdateRoleById(logger, entity); err != nil {
		logger.Warn("DeleteRole UpdateRoleById sql error:", roleId, err.Error())
		return err
	}
	return nil
}
