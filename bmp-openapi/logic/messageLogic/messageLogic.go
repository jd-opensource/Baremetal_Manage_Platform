...
var ShowZh2Types = map[string]string{
	...
	//系统消息类型
	"许可证到期": messageType.MessageSubTypeLicenseNearTimeout,
	//监控消息类型
...
func AddLicenseNoticeMessage(logger *log.Logger, webMsg messageDao.WebMessage) error {

	if _, err := messageDao.AddWebMessage(logger, &webMsg); err != nil {
		logger.Warnf("SendWebNotice error, msg:%s, error:%s", util.InterfaceToJson(webMsg), err.Error())
		return err
	}
	logger.Infof("SendWebNotice success, msg:%s", util.InterfaceToJson(webMsg))
	return nil
...
func GetPageMessages(logger *log.Logger, param *requestTypes.QueryMessagesRequest, p util.Pageable) ([]*response.Message, int64, error) {
	...
	logger.Infof("debug minping, entityList2:%s", util.InterfaceToJson(entityList))
	return res, cnt, nil
...
func DialMail(logger *log.Logger, param *requestTypes.MailDialRequest) error {

	port, err := strconv.Atoi(param.ServerPort)
	if err != nil {
		logger.Warnf("param.server_port error:%s", err.Error())
		return err
	}

	var isPass string

	checkErr := mailLogic.DialMail(logger, param.ServerAddr, port, param.AdminAddr, param.AdminPass, param.To, mailLogic.MailDialTestSubjest, mailLogic.MailDialTestContent)
	if checkErr == nil {
		logger.Info("dialMail.send ok")
		isPass = "1"
	} else {
		logger.Warnf("dialMail.send error:%s", checkErr.Error())
		isPass = "0"
	}

	_, err = SaveMail(logger, param, isPass)
	if err != nil {
		logger.Warnf("DialMail.SaveMail error:%s", err.Error())
	}

	return checkErr
}

func SaveMail(logger *log.Logger, param *requestTypes.MailDialRequest, isPass string) (bool, error) {

	mailEntity, err := mailDao.GetMailMessage(logger)
	if err != nil {
		logger.Warnf("SaveMail.GetMailMessage error:%s", err.Error())
	}
	if mailEntity == nil {
		m := mailDao.MailMessage{
		...
		_, err := mailDao.AddMailMessage(logger, &m)
		if err != nil {
			logger.Warnf("SaveMail.AddMailMessage error:%s", err.Error())
			return false, err
		}
	} else {
		mailEntity.ServerAddr = param.ServerAddr
		mailEntity.ServerPort = param.ServerPort
		mailEntity.AdminAddr = param.AdminAddr
		mailEntity.AdminPass = param.AdminPass
		mailEntity.To = param.To
		mailEntity.IsPass = isPass
		err := mailDao.UpdateMailAnywhere(logger, mailEntity)
		if err != nil {
			logger.Warnf("SaveMail.UpdateMailAnywhere error:%s", err.Error())
			return false, err
		}
	}
...
func GetMail(logger *log.Logger) (*responseTypes.Mail, error) {

	m, err := mailDao.GetMailMessage(logger)
	if err != nil {
		logger.Warnf("GetMail.GetMailMessage error:%s", err.Error())
	}
	if m == nil {
		return nil, err
	}
	return &responseTypes.Mail{
...
func SaveIsPushMail(logger *log.Logger, param *requestTypes.SaveIsPushMailRequest) (bool, error) {
	...
	if mailEntity == nil {
		...
		_, err := mailDao.AddMailMessage(logger, &m)
		if err != nil {
			logger.Warnf("SaveIsPushMail.AddMailMessage error:%s", err.Error())
			return false, err
		}
	} else {
		mailEntity.IsPush = param.IsPush
		err := mailDao.UpdateMailAnywhere(logger, mailEntity)
		if err != nil {
			logger.Warnf("SaveIsPushMail.UpdateMailAnywhere error:%s", err.Error())
			return false, err
		}
	}
