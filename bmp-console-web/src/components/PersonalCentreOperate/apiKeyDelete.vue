        ...
        >
            <template #title>
                <div>
                    ...
                    <span class="my-title">{{$t('personCentre.operate.deleteApiKey')}}</span>
                    <el-button     
                        circle 
                        class="close-button" 
                        :icon="Close"
            ...
            <div class="dialog-body-content">{{$t('personCentre.content.deleteApiKey', [instanceInfo?.name])}}</div>
            <template #footer>
                <span>
                    <el-button 
                        class="cancel-button" 
...
import {apiKeyDeleteAPI} from 'api/request.ts'; // api密钥删除接口
/**
 * 父组件传递的props数据类型
*/
interface PropsType {
...
const requestDelete = (): void => {
    apiKeyDeleteAPI({
        apikeyId: props.instanceInfo.apiKeyId
    }).then(({data}: any) => {
