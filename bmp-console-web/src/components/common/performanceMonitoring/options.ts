    ...
    return new Promise((resolve) => {
        resolve({
            ...
            tooltip: {
                ...
                formatter: function (params: {seriesName: string; value: number; color: string; axisValue: string;}[]) {
                    ...
                    for (let i = 0, l = params.length; i < l; i++) {
                        ...
                        const seconds = yearSecond.getSeconds() < 10 ? '0' + yearSecond.getSeconds() : yearSecond.getSeconds();
                        res += `<p>${yearSecond.getFullYear()}-${monthDateTime.split(',').reverse()[0] + ' ' + monthDateTime.split(',').reverse()[1] + ':' + seconds}</p>`
            ...
            yAxis: {
                ...
                min: isMultiDimensional(args[3]) ? 0 : args[3].every((item: number) => item <= 0) ? '' : args[3].every((item: number) => item < 100) ? 0 : Math.min(...args[3]),
                max: isMultiDimensional(args[3]) ? '' : args[3].every((item: number) => item <= 0) ? '' : args[3].every((item: number) => item < 100) ? 100 : '',
...
const setFormAtter = (args: any, value: number) => {
    const objData = [
        ...
        [
            ...
            () => {
                if (value >= 1000) {
                    return reg(addSymbol(parseFloat(String(value / 1000)).toFixed(2))) + 'K';
                }
...
// const setData = (data: number[][]): number[] => {
//     let newData: number[] = [];
//     data.forEach((item) => {
//         newData.push(...item)
//     })
...
const isMultiDimensional = (arr: string[] | number[][]) => {
    return Array.isArray(arr) && arr.some((item: any) => Array.isArray(item));
}
...
const echartsOpt = (res: any, type: string, myChart: any) => {
    ...
    if (myChart === void 0) {
        ...
        newMyChart = myChart;
        // window.addEventListener('resize', () => size(myChart))
        // window.removeEventListener('resize', () => size(myChart))
    }
