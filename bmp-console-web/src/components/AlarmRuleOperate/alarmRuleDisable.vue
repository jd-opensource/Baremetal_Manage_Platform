        ...
        >
            <template #title>
                <div>
                    ...
                    <span class="my-title">{{$t('alarm.operate.disable.dialogName')}}</span>
                    <el-button     
                        circle 
                        class="close-button" 
                        :icon="Close"
            ...
            <div class="dialog-body-content">{{$t('alarm.operate.disable.dialogContent', [instanceInfo?.ruleName])}}</div>
            <template #footer>
                <span>
                    <el-button      
                        class="cancel-button"
...
import {ruleDisableAPI} from 'api/request.ts'; // 打开接口
/**
 * 父组件传递的props数据类型
*/
interface PropsType {
