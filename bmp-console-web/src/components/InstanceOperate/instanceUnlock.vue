        ...
        >
            <template #title>
                <div>
                    ...
                    <span class="my-title">{{$t('list.operate.unlock')}}</span>
                    <el-button     
                        circle 
                        class="close-button" 
                        :icon="Close"
            ...
            <div class="dialog-body-content">{{$t('list.content.unlock', [instanceInfo?.instanceName])}}</div>
            <template #footer>
                <span>
                    <el-button      
                        class="cancel-button"
...
import {instanceUnlockAPI} from 'api/request.ts'; // 解除锁定接口
/**
 * 父组件传递的props数据类型
*/
interface PropsType {
