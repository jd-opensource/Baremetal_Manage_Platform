        ...
        >
            <template #title>
                <div>
                    ...
                    <span class="my-title">{{$t('list.operate.close')}}</span>
                    <el-button     
                        circle 
                        class="close-button" 
                        :icon="Close"
            ...
            <div class="dialog-body-content">{{$t('list.content.close', [instanceInfo?.instanceName])}}</div>
            <template #footer>
                <span>
                    <el-button      
                        class="cancel-button"
...
import {instanceCloseAPI} from 'api/request.ts'; // 关闭接口
/**
 * 父组件传递的props数据类型
*/
interface PropsType {
