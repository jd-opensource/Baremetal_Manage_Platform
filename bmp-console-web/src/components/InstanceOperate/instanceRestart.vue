        ...
        >
            <template #title>
                <div>
                    ...
                    <span class="my-title">{{$t('list.operate.restart')}}</span>
                    <el-button     
                        circle 
                        class="close-button" 
                        :icon="Close"
            ...
            <div class="dialog-body-content">{{$t('list.content.restart', [instanceInfo?.instanceName])}}</div>
            <template #footer>
                <span>
                    <el-button      
                        class="cancel-button"
...
import {instanceRestartAPI} from 'api/request.ts'; // 重启接口
/**
 * 父组件传递的props数据类型
*/
interface PropsType {
