        ...
        >
            <template #title>
                <div>
                    ...
                    <span class="my-title">{{$t('list.operate.lock')}}</span>
                    <el-button     
                        circle 
                        class="close-button" 
                        :icon="Close"
            ...
            <div class="error-tip-content">
                ...
                {{$t('list.content.lockTip')}}
            </div>
            <div class="dialog-body-content">{{$t('list.content.lock', [instanceInfo?.instanceName])}}</div>
            <template #footer>
                <span>
                    <el-button      
                        class="cancel-button"
...
import {instanceLockAPI} from 'api/request.ts'; // 锁定接口
/**
 * 父组件传递的props数据类型
*/
interface PropsType {
