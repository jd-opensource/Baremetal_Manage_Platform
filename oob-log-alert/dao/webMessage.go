...
func AddWebMessage(logger *log.Logger, m *WebMessage) (id int64, err error) {
	...
	return CreateAndGetId(logger, IronicWdb, m)
}
...
func GetWebMessageById(logger *log.Logger, id int64) (v *WebMessage, err error) {
	v = &WebMessage{}
	err = Where(logger, IronicRdb, "id = ? and is_del = 0", id).Take(v).Error
	if err != nil {
		return nil, err
	}
	return v, nil
...
func GetAllWebMessagesByUserId(logger *log.Logger, userId string) (ml []*WebMessage, err error) {
	...
	var db = Model(logger, IronicRdb, WebMessage{})
...
func UpdateOsById(logger *log.Logger, m *WebMessage) (err error) {
	...
	//m.UpdateTime = time.Now()
	return Model(logger, IronicWdb, WebMessage{}).Where("id = ?", m.ID).Updates(m).Error
}

func GetInstanceBySn(logger *log.Logger, sn string) (*Instance, error) {
	deviceEntity, err := GetCpsBySn(logger, sn)
	if err != nil {
		logger.Warnf("GetInstanceBySn.GetCpsBySn error, sn:%s, error:%s", sn, err.Error())
		return nil, err
	}
	instanceEntity, err := GetInstanceById(logger, deviceEntity.InstanceID)
	if err != nil {
		logger.Warnf("GetInstanceBySn.GetInstanceById error, instance_id:%s, error:%s", deviceEntity.InstanceID, err.Error())
		return nil, err
	}
...
func InsertMessage(logger *log.Logger, message *WebMessage) (err error) {

	return Create(logger, IronicWdb, message).Error
