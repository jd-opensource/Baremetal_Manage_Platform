...
func CreateIdc(logger *log.Logger, m *Idc) (id int64, err error) {
	return CreateAndGetId(logger, IronicRdb, m)
}
...
func GetIdcById(logger *log.Logger, idcId string) (v *Idc, err error) {
	v = &Idc{}
	err = Where(logger, IronicRdb, "idc_id = ? and is_del = 0", idcId).Take(v).Error
	if err != nil {
		return nil, err
	}
	return v, nil
...
func GetOneIdc(logger *log.Logger, query map[string]interface{}) (l *Idc, err error) {
	l = &Idc{}
	var db = Model(logger, IronicRdb, Idc{})
...
func UpdateIdcById(logger *log.Logger, m *Idc, dcId string) (err error) {
	return Model(logger, IronicWdb, Idc{}).Where("idc_id = ?", dcId).Save(m).Error
}

//func DeleteIdcById(logger *log.Logger, m *Idc) (err error) {
//	m.UpdatedBy = logger.GetPoint("username").(string)
//	//m.UpdatedAt = time.Now().Unix()
//	//m.De = time.Now().Unix()
//	return Model(logger, IronicWdb, Idc{}).Where("idc_id = ?", m.DcId).Update(m).Error
//}
