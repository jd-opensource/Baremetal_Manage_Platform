...
// GetNetInfo 根据cidr获取网关和掩码信息
func GetNetInfo(cidr string) (string, string) {
	_, ipNet, _ := net.ParseCIDR(cidr)
	mask := getIpMask(ipNet.Mask)
	gateway := getDefaultGateway(ipNet.IP.String())
	return gateway, mask
}

// getIpMask 获取ip掩码
func getIpMask(mask net.IPMask) string {
...
func getDefaultGateway(ip string) string {
	ipArr := strings.Split(ip, ".")
	if len(ipArr) > 3 {
		lastNum, _ := strconv.Atoi(ipArr[3])
		return ipArr[0] + "." + ipArr[1] + "." + ipArr[2] + "." + strconv.Itoa(lastNum+1)
	} else {
		return ""
	}
}
