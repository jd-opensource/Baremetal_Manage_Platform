...
func UpdaterInstanceSshkeyById(logger *log.Logger, m *RInstanceSshkey) (err error) {
	//m.UpdateTime = time.Now()
	return dao.Model(logger, dao.IronicWdb, RInstanceSshkey{}).Where("instance_id = ? and is_del = 0", m.InstanceID).Update(m).Error

}

func DeleteInstanceSshkeyByInstanceID(logger *log.Logger, instanceID string) (err error) {
	//m.UpdateTime = time.Now()
	return dao.Model(logger, dao.IronicWdb, RInstanceSshkey{}).Where("instance_id = ? and is_del = 0", instanceID).Update("is_del", 1).Error
}
