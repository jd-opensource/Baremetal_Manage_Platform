...
func AddMultiRInstanceVolumeRaid(logger *log.Logger, m []*RInstanceVolumeRaid) (id int64, err error) {
...
func GetRInstanceVolumeRaidByInstanceId(logger *log.Logger, instanceId string) (ml []*RInstanceVolumeRaid, err error) {
	ml = []*RInstanceVolumeRaid{}
	err = dao.Where(logger, dao.IronicRdb, "instance_id = ? and is_del = 0", instanceId).Find(&ml).Error
	if err != nil {
		return nil, err
