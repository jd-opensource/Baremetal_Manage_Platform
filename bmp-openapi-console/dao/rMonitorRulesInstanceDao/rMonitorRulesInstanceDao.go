...
func DeleteMonitorRuleInstanceByInstanceID(logger *log.Logger, instanceID string) (err error) {
	//m.UpdateTime = time.Now()
	return dao.Model(logger, dao.IronicWdb, RMonitorRulesInstance{}).Where("instance_id = ? and is_del = 0", instanceID).Update("is_del", 1).Error
}
...
func GetAllRMonitorRulesInstanceEffective(logger *log.Logger) (ml []*RuleInstance, err error) {

	sql := `select j.rule_id ,j.rule_name, j.instance_id, j.instance_name, i.status FROM monitor_rules i INNER JOIN r_monitor_rules_instance j ON i.rule_id = j.rule_id where 1=1 and i.is_del = 0 and j.is_del = 0 and i.status in (1, 3)`
