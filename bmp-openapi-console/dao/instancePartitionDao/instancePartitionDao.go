...
func AddMultiInstancePartition(logger *log.Logger, m []*InstancePartition) (id int64, err error) {

	tx := dao.GetGormTx(logger)
	tx.Begin()
	for _, instancePartition := range m {
		if err := tx.Create(instancePartition).Error; err != nil {
			tx.Rollback()
			return 0, err
		}
	}
...
func UpdateInstancePartitionById(logger *log.Logger, m *InstancePartition) (err error) {
	return dao.Model(logger, dao.IronicWdb, InstancePartition{}).Where(logger, dao.IronicRdb, "id = ?", m.ID).Updates(m).Error
}

func GetBySn(logger *log.Logger, sn string) (ml []*InstancePartition, err error) {
...
func GetByInstanceId(logger *log.Logger, instanceId string) (ml []*InstancePartition, err error) {
	ml = []*InstancePartition{}
...
func DeletePartitionsByInstanceId(logger *log.Logger, instanceId string) (err error) {
