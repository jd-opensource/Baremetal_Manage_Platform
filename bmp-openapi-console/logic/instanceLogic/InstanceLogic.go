...
func InstanceEntity2Instance(logger *log.Logger, o *instanceDao.Instance, vrs []*response.InstanceVolumeRaid) *responseTypes.Instance {
	...
	v := &responseTypes.Instance{
		...
		IloIP:           device.IloIP,
		Mask:            device.Mask,
		Eth1Mask:        device.MaskEth1,
		PrivateIPv4:     device.PrivateIPv4,
		PrivateEth1IPv4: device.PrivateEth1IPv4,
		PrivateEth1IPv6: device.PrivateEth1IPv6,
		...
		BootMode:    o.BootMode,
		IsNeedRaid:  deviceType.IsNeedRaid,
	}
	if language == baseLogic.EN_US {
		v.StatusName = baseLogic.InstanceStatusEn[o.Status]
	}

	logger.Infof("instance_info:%s", util.InterfaceToJson(v))
...
func GetInstanceById(logger *log.Logger, InstanceId string) (*responseTypes.Instance, error) {
	...
	if err != nil {
		logger.Warn("GetInstanceByUuid sql error:", InstanceId, err.Error())
	...
	for _, volumeEntity := range volumeEntities {
		if language != baseLogic.EN_US {
			if volumeEntity.DiskType == "notLimited" {
				volumeEntity.DiskType = "无限制"
			}
			if volumeEntity.InterfaceType == "notLimited" {
				volumeEntity.InterfaceType = "无限制"
			}
...
func CreateInstance(logger *log.Logger, param requestTypes.CreateInstanceRequest) (ids []string, err error) {
	...
	if project != nil && project.UserID != logger.GetPoint("userId").(string) { //项目要么是自己所有的，要么是共享的
		s, _ := sharingProjectDao.GetOneSharingProject(logger, project.ProjectID, project.UserID, logger.GetPoint("userId").(string))
		if s == nil {
	...
	imageEntity, err := imageLogic.GetByImageId(logger, param.ImageID)
	if err != nil {
	...
	if deviceType.IsNeedRaid != "no_need_raid" { //配置了阵列卡，需要做raid(包括NORAID)
		_, err = raidLogic.GetRaidEntityByRaidId(logger, param.SystemVolumeRaidID)
		if err != nil {
		...
		systemRolumeRaidEntities, err := rVolumeRaidDao.GetSystemVolumeRaid(logger, param.DeviceTypeID)
		if err != nil {
			logger.Warnf("createInstance.GetSystemVolumeRaid error, device_type_id:%s, error:%s", param.DeviceTypeID, err.Error())
			panic(constant.BuildInvalidArgumentWithArgs("此机型系统盘和raid未绑定", "devicetype systemdisk and raid do not associated"))
		}
		selected := false
		for _, v := range systemRolumeRaidEntities {
			if v.RaidID == param.SystemVolumeRaidID {
				selected = true
				break
			}
		}
		if !selected {
			logger.Warnf("此机型系统盘和raid未绑定, error, device_type_id:%s, raid_id:%s", param.DeviceTypeID, param.SystemVolumeRaidID)
			panic(constant.BuildInvalidArgumentWithArgs("此机型系统盘和raid未绑定", "devicetype systemdisk and raid do not associated"))
		}
	}
	...
	var bootmode string = param.BootMode //优先取用户指定的
	if strings.ToLower(imageEntity.Format) == "qcow2" {
		//qcow2镜像没有分区信息,通常给windows使用
	} else { //tar包镜像
	...
	// deviceCount, err := deviceDao.GetDeviceCount(logger, map[string]interface{}{
	// 	"is_del": 0,
	// })
	// if err != nil {
	// 	logger.Warnf("createInstance.GetDeviceCount error:%s", err.Error())
	// }
	...
	for i := 0; i < param.Count; i++ {
		...
		if param.InstallBmpAgent {
			entity.IsInstalledAgent = "1"
		} else {
			entity.IsInstalledAgent = "0"
		}
		...
		for i := 0; i < len(partitionsObj); i++ { //数组的结构体partitionObj，不能用range遍历？？
			...
			if strings.EqualFold(partitionsObj[i].MountPoint, "/") {
				partitionType = "root"
			} else if strings.EqualFold(partitionsObj[i].MountPoint, "/boot") {
				partitionType = "boot"
			} else {
			...
			if strings.EqualFold(partitionsObj[i].MountPoint, "/") {
				partitionLabel = "l_root"
			} else if strings.EqualFold(partitionsObj[i].MountPoint, "swap") {
				partitionLabel = "l_swap"
			} else {
				label := strings.Replace(partitionsObj[i].MountPoint, "/", "l_", -1)
			...
			_, err = instancePartitionDao.AddInstancePartition(logger, &instancePartitionDao.InstancePartition{
				...
				UpdatedBy:           logger.GetPoint("username").(string),
				UpdatedTime:         int(time.Now().Unix()),
				InstancePartitionID: commonUtil.GetRandString("ipt-", namespacePrefix.IMAGE_ID_LENGTH, false, true, true),
				Number:              i + 1,
			})
			if err != nil {
		...
		if deviceType.IsNeedRaid != "no_need_raid" { //配置了阵列卡，需要做raid(包括NORAID)
			systemVolumeEntity, err := volumeDao.GetSystemVolumeByDeviceTypeId(logger, entity.DeviceTypeID)
			if err != nil {
				logger.Warnf("createInstance.GetSystemVolumeByDeviceId error, device_type_id:%s, error:%s", entity.DeviceTypeID, err.Error())
			}

			raidEntity, err := raidDao.GetRaidByUuid(logger, param.SystemVolumeRaidID)
			if err != nil {
				logger.Warnf("createInstance.GetRaidByUuid error, raid_id:%s, error:%s", param.SystemVolumeRaidID, err.Error())
			}
			rInstanceVolumeRaidDaoEntities := []*rInstanceVolumeRaidDao.RInstanceVolumeRaid{
				&rInstanceVolumeRaidDao.RInstanceVolumeRaid{
					InstanceID: entity.InstanceID,
					VolumeID:   systemVolumeEntity.VolumeID,
					VolumeType: systemVolumeEntity.VolumeType,
					RaidID:     param.SystemVolumeRaidID,
					RaidCan:    "", //TODO
					RaidName:   raidEntity.Name,
				},
			}
			for _, v := range param.VolumeRaIDs {
				if v.VolumeID == "" {
					logger.Warnf("createInstance.VolumeRaIDs.volumeId empty, instance_id:%s", entity.InstanceID)
					continue
				}

				if v.VolumeType == "system" {
					continue //TODO
				}
				raidEntity, err := raidDao.GetRaidByUuid(logger, v.RaidID)
				if err != nil {
					logger.Warnf("createInstance.GetRaidByUuid error, raid_id:%s, error:%s", v.RaidID, err.Error())
				}
				e := &rInstanceVolumeRaidDao.RInstanceVolumeRaid{
					InstanceID: entity.InstanceID,
					VolumeID:   v.VolumeID,
				...
				rInstanceVolumeRaidDaoEntities = append(rInstanceVolumeRaidDaoEntities, e)
			}
			if len(rInstanceVolumeRaidDaoEntities) > 0 {
				if _, err := rInstanceVolumeRaidDao.AddMultiRInstanceVolumeRaid(logger, rInstanceVolumeRaidDaoEntities); err != nil {
					logger.Warnf("createInstance.AddMultiRInstanceVolumeRaid error, instance_id:%s, error:%s", entity.InstanceID, err.Error())
				}
			}
		}
...
func afterCreateInstances(logger *log.Logger, entities []*instanceDao.Instance, param requestTypes.CreateInstanceRequest) ([]string, error) {
	...
	userData := getInstanceUserData(logger, param, param.InstallBmpAgent)

	err := sendCreateInstanceEvent(logger, instance_ids, param.Password, userData) //param.UserData, param.AliasIPs, param.ExtensionAliasIps
	if err != nil {
		return nil, err
	}
	if len(entities) == 1 {
...
func sendCreateInstanceEvent(logger *log.Logger, instance_ids []string, password string, userData string) error {
...
func LockInstance(logger *log.Logger, instanceId string) error {
	...
	deviceEntity, err := deviceDao.GetDeviceById(logger, instance.DeviceID)
	if err != nil {
		logger.Warnf("LockInstance.GetDeviceById sql error, device_id:%s, error:%s", instance.DeviceID, err.Error())
		panic(constant.BuildInvalidArgumentWithArgs(constant.NOT_FOUND.Messagech, constant.NOT_FOUND.MessageEn))
	}
	...
	auditLogLogic.SaveAuditLogs(logger, deviceEntity.DeviceID, instanceId, AuditLogsType.AuditLogsTypeLockInstances)
	return nil
}
...
func DeleteInstance(logger *log.Logger, instanceId string) error {
	...
	if entity.Status == instanceStatus.CREATE_ERROR { //创建错误的实例，不用走完整的删除逻辑，直接释放各种资源就行
		...
		if err := deviceHintsDao.DeleteDeviceHintsBySn(logger, deviceEntiry.Sn); err != nil {
			logger.Warn("DeleteInstance DeleteRootDeviceHintsBySn sql error:", instanceId, err.Error())
			return err
		}
	...
	if err := rMonitorRulesInstanceDao.DeleteMonitorRuleInstanceByInstanceID(logger, entity.InstanceID); err != nil {
		logger.Warn("DeleteInstance DeleteMonitorRuleInstanceByInstanceID sql error:", instanceId, err.Error())
	}
...
func GetInstanceList(logger *log.Logger, param *requestTypes.QueryInstancesRequest, p util.Pageable) ([]*responseTypes.Instance, int64, error) {
	...
	var profileInstanceIds []string
	projectEntity, err := projectDao.GetProjectById(logger, param.ProjectID)
	if projectEntity.UserID != userId { //非所属关系，看是否为被共享关系
		q1 := map[string]interface{}{
			"project_id":     param.ProjectID,
			"shared_user_id": userId,
			"is_del":         0,
		}
		sharesProjects, _ := sharingProjectDao.GetAllShareProjects(logger, q1)
		if len(sharesProjects) > 0 {
			pInstanceIds := sharesProjects[0].SharedInstanceIDs
			profileInstanceIds = strings.Split(pInstanceIds, ",")

		} else { //project既不归属于当前用户，又没有共享给当前用户
			logger.Warnf("GetInstanceList project_id validate error, project_id:%s, user_id:%s", param.ProjectID, userId)
			return nil, 0, err
		}
	}
	...
	// 	"is_del":        0,
	// }
	// sharesProjects, _ = sharingProjectDao.GetAllShareProjects(logger, q2)
	// sharedids := []string{}
	// for _, v := range sharesProjects {
	// 	sharedids = append(sharedids, v.OwnerUserID, v.SharedUserID)
	// }
	...
	query := map[string]interface{}{
		...
		//"sn":              c.GetString("sn"),
		"project_id": param.ProjectID,
		// "user_id.in": append(append([]string{userId}, ownerids...), sharedids...), //当前用户可以是共享用户，也可以是owner用户
		"idc_id": param.IdcID,
	}
	if param.DeviceID != "" {
	...
	if len(profileInstanceIds) > 0 {
		query["instance_id.in"] = profileInstanceIds
	}
	...
	if param.IsInstallAgent != "" {
		query["is_installed_agent"] = param.IsInstallAgent
	}
...
//logger里面的userid为当前操作用户的userId,拥有者和共享者都有权限操作
func InstanceOwnedCheck(logger *log.Logger, instance *instanceDao.Instance) bool {
	...
	sha, err := sharingProjectDao.GetAllShareProjects(logger, q)
	if sha != nil && len(sha) != 0 {
		return true
	}
...
func ReinstallInstance(logger *log.Logger, instance_id string, param *requestTypes.ReinstallInstanceRequest) error {
	...
	// if param.SystemVolumeRaidID != "" {
	// 	instance.SystemVolumeRaidID = param.SystemVolumeRaidID
	// }
	...
	//释放销毁实例使用的sshkey资源
	if err := rInstanceSshkeyDao.DeleteInstanceSshkeyByInstanceID(logger, instance_id); err != nil {
		logger.Warn("DeleteInstance ReinstallInstanceSshkeyByInstanceID sql error:", instance_id, err.Error())
	}
	if param.SshKeyID != "" {
		for _, sshkeyId := range strings.Split(param.SshKeyID, ",") {
			sshkeyId = strings.TrimSpace(sshkeyId)
			...
			if err != nil {
				logger.Warnf("add r_instance_sshkey error, instance_id:%s, sshkeyId:%s, error:%s", instance_id, sshkeyId, err.Error())
				return err
			}
		}
	}
...
func GetInstanceListByProjectIdAndOwnerNameAndSharerName(logger *log.Logger, param *requestTypes.DescribeInstancesByProjectIdAndOwnerNameAndSharerNameRequest) ([]*responseTypes.InstanceForShare, error) {
	userId := logger.GetPoint("userId").(string)
	...
	if param.InstanceName != "" {
		q["instance_name.like"] = "%" + param.InstanceName + "%"
	}
	if param.InstanceID != "" {
		q["instance_id.like"] = "%" + param.InstanceID + "%"
	}

	//获取当前用户指定项目下的所有实例列表
	instanceEntities, _ := instanceDao.GetAllInstance(logger, q)

	sharerUser, _ := userLogic.GetUserByName(logger, &requestTypes.GetUserByNameRequest{
		UserName: param.SharerName,
	})
	if sharerUser == nil {
		return nil, errors.New("param sharerName invalided")
	}

	var hasSharedInstanceIds []string

	sharesProject, _ := sharingProjectDao.GetOneSharingProject(logger, param.ProjectID, userId, sharerUser.UserID)
	if sharesProject != nil {
		tt := sharesProject.SharedInstanceIDs
		hasSharedInstanceIds = strings.Split(tt, ",")
	} else { //整个项目没有被共享过
		//empty
	}
	logger.Info("hasSharedInstanceIds is:", hasSharedInstanceIds)
	// q2 := map[string]interface{}{
	// 	"project_id":    param.ProjectID,
	// 	"owner_user_id": userId,
	// 	"is_del":        0,
	...
	instances := InstanceEntitys2ShareInstances(instanceEntities, hasSharedInstanceIds)
	return instances, nil
}

func InstanceEntitys2ShareInstances(entitys []*instanceDao.Instance, hasSharedInstanceIds []string) []*responseTypes.InstanceForShare {
	res1 := []*responseTypes.InstanceForShare{} //已共享
	res2 := []*responseTypes.InstanceForShare{} //未共享
	for _, entity := range entitys {
		v := &responseTypes.InstanceForShare{
			InstanceID:   entity.InstanceID,
			InstanceName: entity.InstanceName,
		}
		if len(hasSharedInstanceIds) == 1 && hasSharedInstanceIds[0] == "" {
			v.HasShared = false
		} else {
			if inArray(v.InstanceID, hasSharedInstanceIds) {
				v.HasShared = true
			}
		}
		if v.HasShared {
