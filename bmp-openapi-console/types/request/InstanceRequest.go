...
type CreateInstanceRequest struct {
	...
	// 系统盘RAID ID
	// required: true
	SystemVolumeRaidID string `json:"systemVolumeRaidId"` //使用盘符时，这个为空
	// 系统盘分区
...
type ReinstallInstanceRequest struct {
	...
	// 系统盘RAID ID
	// required: true
	SystemVolumeRaidID string `json:"systemVolumeRaidId"` //使用盘符装时不传此字段
	// 系统盘分区
	SystemPartition []Partition `json:"systemPartition"`
	// 数据盘分区
...
func (req *DescribeInstancesByProjectIdAndOwnerNameAndSharerNameRequest) Validate(logger *log.Logger) {
	validate(req, logger)
}
