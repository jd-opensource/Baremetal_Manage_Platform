...
func (b *BaseController) CatchException() {
	...
	// }
	if r := recover(); r != nil {
		b.logPoints.Warnf("BaseController.CatchException catch exception, error:%v, stack:%s", r, getCurrentGoroutineStack())
...
func (b *BaseController) ValidateAuth() *constant.RespMsg {
	...
	} else if apiKeyEntity.Type == "alert" { //监控告警系统上报过来的数据
		b.logPoints.Point("traffic_source", "alert-api")
		if b.Ctx.Input.URI() == "/v1/monitorAlert/addAlert" {
			return nil //由业务侧补用户信息
		} else {
			err := constant.NOT_SUPPORTED
			return &err
		}
	...
	if apiKeyEntity.Type == "user" && apiKeyEntity.Source != "console" {
		//个人apikey且不是在console上创建的
		err := constant.PermissionDenyForObject
		return &err
	}
	...
	if u.RoleID != baseLogic.ROLE_ADMIN_UUID {
		if strings.HasPrefix(b.Ctx.Input.URI(), "/v1/roles") && b.logPoints.GetPoint("method").(string) != "GET" {
		...
		if strings.HasPrefix(b.Ctx.Input.URI(), "/v1/users") && b.logPoints.GetPoint("method").(string) != "GET" {
			b.logPoints.Warn("role method for non-admin user invalid, userId:", u.UserID)
			err := constant.PermissionDenyForObject
			return &err
		}
