...
//内部接口，查询Pronoea侧和数据库中规则是否一致
func (c *MonitorRuleController) CheckDiffFromPronoea() {
	defer c.CatchException()
	c.logPoints.Infof("CheckDiffFromPronoea body:%s", string(c.Ctx.Input.RequestBody))

	res, err := monitorRuleLogic.CheckDiffFromPronoea(c.logPoints)
	if err != nil {
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
