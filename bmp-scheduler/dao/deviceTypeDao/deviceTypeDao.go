...
type DeviceType struct {
	...
	CPUAmount                 int8   `gorm:"column:cpu_amount" json:"cpuAmount"`                                   // cpu数量
	CPUCores                  int8   `gorm:"column:cpu_cores" json:"cpuCores"`                                     // 单个cpu内核数
	...
	NicAmount                 int8   `gorm:"column:nic_amount" json:"nicAmount"`                                   // 网卡数量
	...
	SystemVolumeAmount        int8   `gorm:"column:system_volume_amount" json:"systemVolumeAmount"`                // 系统盘数量
	GpuAmount                 int8   `gorm:"column:gpu_amount" json:"gpuAmount"`                                   // gpu数量
	...
	DataVolumeAmount          int8   `gorm:"column:data_volume_amount" json:"dataVolumeAmount"`                    // 数据盘数量
...
func UpdateDeviceTypeByDeviceTypeID(logger *log.Logger, m *DeviceType, deviceTypeID string) (err error) {
	//m.UpdateUser = "admin"
	err = dao.Model(logger, dao.IronicWdb, DeviceType{}).Where("device_type_id = ?", deviceTypeID).Update(m).Error
	return
}
