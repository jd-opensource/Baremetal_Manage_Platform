...
func GetJDBondInterfaceBysn(logger *log.Logger, sn string) (v *JdBondInterface, err error) {
	v = &JdBondInterface{}
...
func UpdateJdBondInterfaceById(logger *log.Logger, m *JdBondInterface) (err error) {
	...
	m.UpdateTime = time.Now()
	return dao.Model(logger, dao.IronicWdb, JdBondInterface{}).Where("id = ?", m.ID).Updates(m).Error
}

func GetOneInterface(logger *log.Logger, query map[string]interface{}) (l *JdBondInterface, err error) {
	l = &JdBondInterface{}
	var db = dao.Model(logger, dao.IronicRdb, JdBondInterface{})
	db, err = dao.WhereBuild(db, query)
	if err != nil {
