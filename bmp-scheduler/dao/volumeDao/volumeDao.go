...
type Volume struct {
	...
	VolumeName    string `gorm:"column:volume_name" json:"volumeName"`       // 卷名称
	VolumeType    string `gorm:"column:volume_type" json:"volumeType"`       // 卷类型：系统卷，数据卷
	DiskType      string `gorm:"column:disk_type" json:"diskType"`           // 硬盘类型（SSD,HDD）
	InterfaceType string `gorm:"column:interface_type" json:"interfaceType"` // 接口类型（SATA,SAS,NVME,不限制）
	VolumeSize    string `gorm:"column:volume_size" json:"volumeSize"`       // 单盘大小（最小容量）
	VolumeUnit    string `gorm:"column:volume_unit" json:"volumeUnit"`       // 硬盘单位（GB,TB）
	VolumeAmount  int    `gorm:"column:volume_amount" json:"volumeAmount"`   // 硬盘数量（最低块数）
...
func GetSystemVolumeByDeviceTypeId(logger *log.Logger, deviceTypeId string) (v *Volume, err error) {
	v = &Volume{}
	err = dao.Where(logger, dao.IronicRdb, "volume_id = ? and volume_type = 'system' and is_del = 0", deviceTypeId).Take(v).Error
	if err != nil {
		return nil, err
	}
	return v, nil
