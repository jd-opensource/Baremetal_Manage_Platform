...
type Interface struct {
	...
	SwitchPort    string    `gorm:"column:switch_port;type:varchar(32)" json:"switch_port"`             // 内网交换机Port
...
func AddMultiInterface(logger *log.Logger, m []*Interface) (id int64, err error) {

	tx := dao.GetGormTx(logger)
	tx.Begin()
	for _, interf := range m {
		interf.CreateTime = time.Now()
		interf.UpdateTime = time.Now()
		if err := tx.Create(interf).Error; err != nil {
			tx.Rollback()
			return 0, err
		}
	}
...
func GetInterfacesBySn(logger *log.Logger, sn string) (ml []*Interface, err error) {
	var db = dao.Model(logger, dao.IronicRdb, Interface{})
	err = db.Where("sn = ? and is_del = ？", sn, 0).Order("id asc").Find(&ml).Error
	return
}
...
func UpdateInterfaceById(logger *log.Logger, m *Interface) (err error) {
	...
	m.UpdateTime = time.Now()
	return dao.Model(logger, dao.IronicWdb, Interface{}).Where("id = ?", m.ID).Updates(m).Error
}
