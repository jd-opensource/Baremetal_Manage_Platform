...
func GetOneUnfinishedAuditLogs(logger *log.Logger, sn, instanceId, operation string) (l *AuditLogs, err error) {
	v := &AuditLogs{}
	err = dao.Where(logger, dao.IronicRdb, "is_del = 0 and instance_id = ? and sn = ? and operation = ? and is_del = 0 and result = 'doing'", instanceId, sn, operation).Take(v).Error
	if err != nil {
		return nil, err
	}
	return v, nil
