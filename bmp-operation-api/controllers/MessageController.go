...
import (
	...
	"coding.jd.com/aidc-bmp/bmp-operation-api/logic/messageLogic"
...
func (c *MessageController) DialMail() {
	...
	res, err := messageLogic.DialMail(c.logPoints, req)
	if err != nil {
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
...
func (c *MessageController) SaveIsPushMail() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, req); err != nil {
		c.logPoints.Warn("SaveIsPushMail parse MailSaveIsPushRequest body error:", err.Error())
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
...
func (c *MessageController) DescribeMail() {
	defer c.CatchException()

	res, err := messageLogic.DescribeMail(c.logPoints)
	if err != nil {
		c.SetErrorResponse(httpStatus.BAD_REQUEST, err.Error(), errorCode.INVALID_ARGUMENT)
		return
	}
