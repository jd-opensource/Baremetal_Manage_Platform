...
func UtcToTime(utc string) string { //2022-02-18T06:01:32Z  这个时间转为东八区 为2022-02-18 14:01:32
	return utc
	if utc == "" {
		return ""
	}
	if !strings.Contains(utc, "T") {
		return utc
	}
	ctime, _ := time.ParseInLocation("2006-01-02T15:04:05Z", utc, time.UTC) //如果utc是带有TZ的，说明是UTC时间，先转成UTC时间
	return ctime.Local().Format(FormatTime)
}
