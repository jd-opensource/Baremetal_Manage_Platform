import {RuleFormRefType, SuccessType} from '@utils/publicType';
// 
...
import {editModelAPI} from 'api/model/request.ts'; // 添加机型接口
...
const formRules = () => {
    ...
    const remove = new StorageOperate(newVal);
    ...
    const requestEditModel = (cpuBol: boolean, memBol: boolean) => {
        ...
        const volumesData = arrayCard === '1' ? volumeManagerTableData.map((item: any) => {
            ...
            return {
                ...
                volumeName: item.volumeName,
                volumeType: setVolumeType(item.newVolumeType),
                // item.newVolumeType === language.t('modelForm.label.system') ? 'system' : 'data',
                diskType: item.diskType,
                interfaceType: item.interfaceType,
        ...
        editModelAPI(
            ...
            if(data?.result?.success) {
                ...
                router.go(-1)
                remove.remove();
                methodsTotal.sendMsg('model-edit-success', 'success');
            }
        }).finally(() => {
    ...
    */
    ...
    // const totalDataSave = dataCount.ruleForm.volumeManagerTableData.every((item: {editFlag: boolean}) => item.editFlag);
