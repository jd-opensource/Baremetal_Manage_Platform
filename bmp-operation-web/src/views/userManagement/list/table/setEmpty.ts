...
class SetEmpty {
    ...
    watchTableData = (props: PropsType) => {
        watch(() => props?.userList?.reactiveArr.tableData, (newValue) => {
            if (!newValue?.length) {
                nextTick(() => {
                    this.textEmpty();
                })
    ...
    textEmpty = () => {
        ...
        ai!.innerHTML = '';
        const customText = document.querySelector('.user-list .el-scrollbar__view');
        ...
        const str = `
            ...
            <span class="empty-text">
                ${language.t('userList.operate.addUser')}
            </span>
        `
        ...
        const clickOpt = document.querySelector('.user-list .custom-tip-count .empty-text');
