...
import {monitorDataAPI, desrcibeTagsAPI} from 'api/device/request.ts';
...
class EchartsOpt extends RuleFormOpt {
    ...
    constructor(props: any, fn: Function) {
        super();
        const that = this;
    ...
    watchValue1 = () => {
        watch(() => this.value1, (newValue) => {
            ...
            if (newValue.value?.length) {
                ...
                let str: string = '';
                let newValS: string = '';
                if (Object.is(this.readioSearch.value, language.t('monitorEcharts.radio.instance'))) {
                    str = '';
                }
                else {
    ...
    watchDetail = () => {
        watch(() => this.props.deviceDetail.reactiveArr.detail, () => {
            this.initData();
        }, {
            deep: true
        })
    ...
    watchRadio = () => {
        watch(() => this.readioSearch, (newValue: {value: string}) => {
            const objData = [
                [
                    (value: string) => Object.is(value, language.t('monitorEcharts.radio.instance')),
                    () => {
                        this.diskVal.value = '';
                        this.nicVal.value = '';
                ...
                [
                    (value: string) => Object.is(value, language.t('monitorEcharts.radio.disk')),
                    (value: string) => {
                        this.nicVal.value = '';
                        this.mountpointVal.value = '';
                        this.initTagName(this.getTagName(value)!, 1)
                    }
                ],
                [
                    (value: string) => Object.is(value, language.t('monitorEcharts.radio.diskPartition')),
                    (value: string) => {
                        this.diskVal.value = '';
                        this.nicVal.value = '';
                        this.initTagName(this.getTagName(value)!, 2)
                    }
                ],
                [
                    (value: string) => Object.is(value, language.t('monitorEcharts.radio.netWorkInterface')),
                    (value: string) => {
                        this.diskVal.value = '';
                        this.mountpointVal.value = '';
                        this.initTagName(this.getTagName(value)!, 3)
                    }
                ]
            ]
            outer: for (const key of objData) {
    ...
    refresh = () => {
        ...
        const objData = [
            ...
            [
                () => Object.is(value, language.t('monitorEcharts.radio.disk')),
                () => this.disk(this.diskVal.value)
            ],
            [
                () => Object.is(value, language.t('monitorEcharts.radio.diskPartition')),
                () => this.mountpoint(this.mountpointVal.value)
            ],
            [
                () => Object.is(value, language.t('monitorEcharts.radio.netWorkInterface')),
    ...
    initTagName = async (tagName: string, value: number) => {
        const queryParams = new URLSearchParams(location.search);
        const instanceId: string = queryParams.get('instanceId') as string;instanceId;
        const that = this;
        const params = {
            instanceId,
            tagName
        ...
        try {
            const res = await desrcibeTagsAPI(params);
            if (res?.data?.result?.length) {
                this.selectValForm[`${tagName}` as keyof typeof that.selectValForm] = res.data.result;
                const v = Object.is(value, 1) ? 'diskVal' : Object.is(value, 2) ? 'mountpointVal' : 'nicVal';
                this[v].value = res.data.result[0];
                // const val = this[value].value;
                const val = this[v].value;
    ...
    selectChange = (val: string, value: string) => {
        ...
        publicConfigOpt.smallBigSet(that as any, val, 'small', value);
    }
}
