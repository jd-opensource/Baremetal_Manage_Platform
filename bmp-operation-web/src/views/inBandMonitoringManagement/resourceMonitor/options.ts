...
const watchNav = () => {
    watch(() => store.navigationBarStatus.hasNavigationBar, () => {
        if (Object.is(myChartD, void 0)) return;
        setTimeout(() => {
            if (myChartD?.length) {
    ...
    return new Promise((resolve) => {
        resolve({
            ...
            yAxis: {
                ...
                // max: isMultiDimensional(args[3]) ? 'dataMax' : args[3].some((item: number) => item > 100) ? 'dataMax' : 100,
                // min: isMultiDimensional(args[3]) ? 0 : args[3].every((item: number) => item < 100) ? 0 : Math.min(...args[3]),
                // max: isMultiDimensional(args[3]) ? '' : args[3].every((item: number) => item < 100) ? 100 : '',
