...
class SetEmpty {
    ...
    constructor(props: any) {
        ...
        this.mitt.instanceMitt?.proxy?.$Bus?.on('history-alarm-list', this.watchTableData);
        onUnmounted(() =>{
            p.innerHTML = '';
            this.mitt.instanceMitt?.proxy?.$Bus?.off('history-alarm-list', this.watchTableData);
        });
    };
    ...
    textEmpty = () => {
        ...
        if (ai === null) return;
        ai!.innerHTML = '';
        const customText: HTMLElement | null = document.querySelector('.history-alarm-list .el-scrollbar__view');
