...
class RulesCheck {
    ...
    errorTip: Ref<string> = ref<string>(language.t('resetPassword.ipt.errorTip.newPassword'));
    ...
    newPasswordCheck: unknown = (_: {[x: string]: unknown;}, value: string, callback: (arg0?: Error | string) => void) => {
        ...
        const passwordArr = [
            [
                ...
                () => {
                    this.errorTip.value = language.t('resetPassword.ipt.emptyTip.newPassword');
                    callback(new Error()); 
                }
            ],
            [
                ...
                () => {
                    this.errorTip.value = language.t('resetPassword.ipt.errorTip.newPassword');
                    callback(new Error());
                }
            ],
            [
    ...
    confirmPasswordCheck: unknown = (_: {[x: string]: unknown;}, value: string, callback: (arg0?: Error | string) => void) => {
        ...
        const confirmPasswordArr = [
            [
                (value: string) => !value?.length,
                () => callback(new Error(language.t('resetPassword.ipt.emptyTip.confirmPassword')))
            ],
            [
                (value: string) => (!RegularContent.passwordReg.test(value)),
                () => callback(new Error(language.t('resetPassword.ipt.errorTip.newPassword')))
            ],
            [
                (value: string) => (value !== this.newPassword.value),
                () => callback(new Error(language.t('resetPassword.ipt.errorTip.confirmPassword')))
            ],
            [
                (value: string) => value,
                () => {
        ...
        newPassword: [ // 邮箱
            {
                required: true,
                trigger: 'blur',
                validator: this.newPasswordCheck
            }
        ],
        confirmPassword: [
            {
