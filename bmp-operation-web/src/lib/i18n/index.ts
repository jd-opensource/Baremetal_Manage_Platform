...
// cookie ts规范校验
const cookie: CookieType = VueCookies;

const isCookie = cookie?.get && cookie.get('X-Jdcloud-Language');
...
const getLocale = () => {
    ...
    // 默认语言
    return isCookie || 'zh_CN';
};

/**
 * 首先从缓存里拿，没有的话就用浏览器语言
*/
const i18n: I18n<any, unknown, unknown, boolean> = createI18n({
    legacy: false, // 使用 vue3 组合式API 时必须设置为 false
    locale: getLocale(), // 本地语言
    globalInjection: true, // 全局注入 $t 函数
    fallbackLocale: isCookie || 'zh_CN', // 设置备用语言
    warnHtmlMessage: false, // 解决v-html警告
    messages // 配置对象
});
